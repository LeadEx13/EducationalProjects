...
void mainmenu()
{
	...
	printf("%s  C)ccc     h)   HH     e)EEEE     c)CCCC    k)  KK     e)EEEE    r)         s)SSSS   %s by Maor and Vlad V1.6\n\n", RED, WHITE);
	bool exit = FALSE;
	while (!exit)
	{
...
//Recursion
void freeall(int size, piece*** board,int gotosize)
{
	if (size == gotosize)
	{
		free(board[0]);
	...
	{
		free(board[gotosize]);
		freeall(size, &board[0],gotosize+1);
	}
	return;
}
//clearing screen
