...
bool Animal::operator==(const Animal& u)//checks if 2 animales are able to mate
{
	if (strcmp(this->get_type(), u.get_type()) == 0)//checks if they bouth are the same kind
	{
		if (sex == u.sex)//checks if they are the same gender
		{
	...
	{
		if ((strcmp(this->get_type(), "Bull") == 0 && strcmp(u.get_type(), "Person") ==0)||(strcmp(this->get_type(), "Person") == 0 && strcmp(u.get_type(), "Bull")==0))//checks if they can make a minotaur
		{
			if (sex == u.sex)//checks if they are the same gender
			{
