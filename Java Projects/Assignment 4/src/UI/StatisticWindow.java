...
public class StatisticWindow extends JDialog {
	...
	public StatisticWindow(JFrame window,Map world,String row_name) {
    	...
    	JPanel p_doses=new JPanel();
    	JButton b_dose = new JButton("Add");
    	JLabel l_dose = new JLabel("Doses:");
    	p_doses.add(l_dose);
    	p_doses.add(spinner);
    	p_doses.add(b_dose);	
    	b_dose.addActionListener(new ActionListener() {
    		public void actionPerformed(ActionEvent e) {
    			int doses = (Integer) spinner.getValue();
    			table_model.setdose(doses);
    		}
    	});
    	JDialog vaccinate=new JDialog(this,"Add vaccinate doses",true);
    	vaccinate.setBounds(390,170,200,300);
    	vaccinate.getContentPane().add(p_doses);
    	vaccinate.pack();
